...
export class mapServerDatabase {
  ...
  constructor(dbPath = SERVER_DB_PATH) {
    ...
    // prevents immediate SQLITE_BUSY errors under concurrent HTTP requests.
  ...
  // Serialize all writes through a promise chain lock.
  // Prevents "cannot start a transaction within a transaction" when
  // concurrent HTTP requests hit saveDeltas at the same time.
...
export const serverDataBase = new mapServerDatabase();
