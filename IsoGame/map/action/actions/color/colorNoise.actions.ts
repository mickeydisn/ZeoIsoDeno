...
import { ShapeConfig, withLine, withShape } from "../../utils/withShape.ts";
...
export const colorNoise = defineAction<"colorNoise", ColorNoiseConfig>(
  ...
  (conf, ctx) => {
    ...
    const base = normaliseColor(
      conf.color ?? tile.color ?? [128, 128, 128, 255],
    );
...
export const colorSmoothShape = defineAction<
  "colorSmoothShape",
  ColorSmoothShapeConfig
>(
  "colorSmoothShape",
  (conf, ctx) => {
