...
export const drawAsset = (
  ...
  try {
    ...
    if (cimage) {
      ...
      const lvl = currentlvl + (itemConf.lvl || 0) * _ctx.conf.mapGridTileScale;
      const p2 = _ctx.isoProject.translatePoint(
        new PointIso(x + off.x, y + off.y, lvl),
      );

      const p = _ctx.isomer.translatePoint(
        new Point(x + off.x, y + off.y, lvl),
      );
      const scale = _ctx.conf.mapGridTileScale;
