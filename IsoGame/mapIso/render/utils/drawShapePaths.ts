...
import { Point } from "@iso-game/mapIso/utils/iso/point.ts";
...
export const drawShapePaths = (
  ...
  if (text && allProjectedPoints.length > 0) {
    ...
    const centerX = allProjectedPoints.reduce((sum, p) => sum + p.x, 0) /
      allProjectedPoints.length;
    const centerY = allProjectedPoints.reduce((sum, p) => sum + p.y, 0) /
      allProjectedPoints.length;
...
export const drawSurfaceFlat = (
  ...
  const shape = Shape.SurfaceFlat(
    new Point(xx, yy, currentlvl),
    1,
    1,
    0, // height,
  );
  drawShapePaths(_ctx, shape, fillColor, text); // Display hover coordinates for debugging
};
