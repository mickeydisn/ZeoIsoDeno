...
import { _drawTileBack, _drawTileFloor, _drawTileFront, _drawTileItem } from "@iso-game/mapIso/_render/isoTileRender.ts";
...
const drawTile = (
  ...
  let offx = (mapState.xf -  mapState.x) / _ctx.conf.SCALE_MOD;
  let offy = (mapState.yf -  mapState.y) / _ctx.conf.SCALE_MOD;
  ...
  if (metaTile.itemsBox) {
    ...
    const p = _ctx.isoProject.tileToScreen(new PointIso(xx, yy, 0))

    _ctx.handler.send(msgToScreen.infoCardPosition({
